...
def test_cable_compute_losses_uses_dense_pre_topk_weights_for_diversity_and_tail_diagnostic():
    ...
    assert torch.allclose(result.metrics["experts/times_engaged/mean"], torch.tensor(5.0))
    assert torch.allclose(result.metrics["experts/times_engaged/std"], torch.tensor(0.0))
    assert torch.allclose(result.metrics["experts/times_engaged/min"], torch.tensor(5.0))
    assert torch.allclose(result.metrics["experts/times_engaged/max"], torch.tensor(5.0))
    assert torch.allclose(result.metrics["experts/num_ever_engaged"], torch.tensor(2.0))
...
def test_cable_weak_compute_losses_returns_scalar_totals_and_metrics():
    ...
    assert torch.allclose(result.metrics["experts/num_engaged/min"], torch.tensor(2.0))
    assert torch.allclose(result.metrics["experts/num_engaged/max"], torch.tensor(2.0))
    assert torch.allclose(result.metrics["experts/times_engaged/mean"], torch.tensor(9.0))
    assert torch.allclose(result.metrics["experts/times_engaged/std"], torch.tensor(0.0))
    assert torch.allclose(result.metrics["experts/times_engaged/min"], torch.tensor(9.0))
    assert torch.allclose(result.metrics["experts/times_engaged/max"], torch.tensor(9.0))
    assert torch.allclose(result.metrics["experts/num_ever_engaged"], torch.tensor(2.0))
