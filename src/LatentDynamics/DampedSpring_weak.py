...
class DampedSpring_weak(LatentDynamics):
    def __init__(   self, 
        ...
        # attributes, along with integration.;
        ...
        # self.coef_norm_order    = config['spring_w']['coef_norm_order'];
        ...
        # self.pq    = config['spring_w']['pq'];
        # self.LS_loss_type    = config['spring_w']['LS_loss_type'];

        # self.dt = 0.002 #need to add this as an argument or in config

        # self.T = self.dt * (self.n_t - 1)
        # self.Phis, self.dPhis, self.d2Phis = self.get_test_functions(self.T, self.n_t, self.test_func_width,self.overlap, self.pq, test_func = self.test_func)
    ...
    def calibrate(self, 
        ...
        if (n_param > 1):
            ...
            for i in range(n_param):
                ...
                """
                result : tuple[torch.Tensor] = self.calibrate(Phis          = [Phis[i]],
                                                              dPhis         = [dPhis[i]],
                                                              d2Phis        = [d2Phis[i]],
        ...
        Gk, bk = self.compute_Gk_bk(self.n_z,Phis,dPhis,d2Phis,Z_1,[Z_D,Z_V]) # second order deriv intead of 1st
        coefs   : torch.Tensor  = torch.linalg.lstsq(Gk, bk).solution;
        coefs = coefs.reshape((self.n_z, Z_1.shape[-1])).T
        ...
        Loss_LD = self.LD_LossFunction(0.5* (d2Phis @ Z_D - dPhis @ Z_V), Phis @ Z_1 @ coefs) #another way of averaging??
