...
class Rollout_2_IC(Trainer):
    def __init__(self, 
        ...
        **Configuration format**
        ...
        - Subclass-specific hyperparameters live under `config['trainer']['Rollout_2_IC']`
        ...
        **Checkpointing**
        ...
        assert config['trainer']['type'] == "Rollout_2_IC", "config['trainer']['type'] = %s, should be Rollout_2_IC" % config['trainer']['type'];
        assert "Rollout_2_IC" in config['trainer'],         "Rollout_2_IC must be in config['trainer']";

        LOGGER.info("Initializing a Rollout_2_IC object"); 

        # Fetch the trainer sub-dictionary.
        trainer_config          : dict      = config['trainer'];
        sub_config              : dict      = trainer_config['Rollout_2_IC'];
        ...
        assert 'n_rollouts' in sub_config, "Rollout_2_IC config must include `n_rollouts` (int > 0) for rollout supervision";
