...
def run(s               = 4.0,   # parameter in initial condition, where v_3(0,x) = (-s/80) sin(s x_1)
        offline         = True,  # Whether to enable the offline phase.
        dt              = 0.01,  # time step
        t_final         = 5.0,   # Final time; start time is 0.
        ode_solver_type = 14,    # see doc string
        id_param        = 2):    # Parametric index

    """
    Projection ROM - MFEM nonlinear elasticity equation example.

    ODE_solver: 1 - Backward Euler, 
                2 - SDIRK2, 
                3 - SDIRK3,
                11 - Forward Euler, 
                12 - RK2,
    ...
    global s_global;
    s_global = s;
    print("IC PARAMETER s = {}\n".format(s))


    mesh_file         = "../data/beam-quad.mesh" # mesh file to use
    ser_ref_levels    = 2     # Number of times to refine the mesh uniformly in serial.
    par_ref_levels    = 0     # Number of times to refine the mesh uniformly in parallel.
    order             = 2     # Order (degree) of the finite elements.
    visc              = 1e-2  # Viscosity coefficient.
    mu                = 0.25  # Shear modulus in the Neo-Hookean hyperelastic model.
    K                 = 5.0   # Bulk modulus in the Neo-Hookean hyperelastic model.
    adaptive_lin_rtol = True  # Enable adaptive linear solver rtol.
    visualization     = False # Enable GLVis visualization.
    visit             = False # Save data files for VisIt (visit.llnl.gov) visualization.
    vis_steps         = 100   # Visualize every n-th timestep.
    nsets             = 0     # Number of parametric snapshot sets
    online            = False # Enable the online phase
    merge             = False # Enable the merge phase.
    use_sopt          = False # Use S-OPT sampling instead of DEIM for the hyperreduction.
    num_samples_req   = -1    # number of samples we want to select for the sampling algorithm.
    rxdim             = -1    # Basis dimension for displacement solution space.
    rvdim             = -1    # Basis dimension for velocity solution space.
    hdim              = -1    # Basis dimension for the nonlinear term.
    hyperreduce       = False # Enable Hyper reduce nonlinear term
    x_base_only       = False # Use the displacement (X) basis to approximate velocity.
    def_ic            = False # Use a deformation initial condition. Default is velocity IC.
    ...
    parallel_mesh = mfem.ParMesh(MPI.COMM_WORLD, mesh)
    del mesh
    ...
    fespace = mfem.ParFiniteElementSpace(parallel_mesh, fe_coll, dim)
    ...
    parallel_mesh.GetNodes(x_ref)

    w_fec = mfem.L2_FECollection(order + 1, dim)
    w_fespace = mfem.ParFiniteElementSpace(parallel_mesh, w_fec)
    ...
    if (visualization):
        ...
        vis_v.precision(8)
        visualize(vis_v, parallel_mesh, x_gf, v_gf, "Velocity", True)
        # // Make sure all ranks have sent their 'v' solution before initiating
        # // another set of GLVis connections (one from each rank):
        parallel_mesh.GetComm().Barrier()
        vis_w = mfem.socketstream(vishost, visport)
        if (vis_w.good()):
            oper.GetElasticEnergyDensity(x_gf, w_gf)
            vis_w.precision(8)
            visualize(vis_w, parallel_mesh, x_gf, w_gf, "Elastic energy density", True)
    ...
    if (visit):
        if (offline):
            dc = mfem.VisItDataCollection("nlelast-fom", parallel_mesh)
        else:
            dc = mfem.VisItDataCollection("nlelast-rom", parallel_mesh)
    ...
    if (online):
        ...
        if (myid == 0):
            ...
            sp_XV_space = mfem.ParFiniteElementSpace(parallel_mesh, fe_coll, dim)
    ...
    while (not last_step):
        ...
        if (online):
            ...
            # Store the updated position + velocity in the corresponding arrays.
            counter                    += 1;
            Velocities[counter, :]      = np.array(vx.GetBlock(0).GetDataArray());
            Displacements[counter, :]   = np.array(vx.GetBlock(1).GetDataArray());
        ...
        if (last_step or ((ti % vis_steps) == 0)):
            ...
            if (visualization):
                visualize(vis_v, parallel_mesh, x_gf, v_gf)
                if (vis_w):
                    oper.GetElasticEnergyDensity(x_gf, w_gf)
                    visualize(vis_w, parallel_mesh, x_gf, w_gf)

            if (visit):
                nodes = x_gf
                owns_nodes = 0
                parallel_mesh.SwapNodes(nodes, owns_nodes)
    ...
    # All done, return the displacement, velocity arrays.
    return Displacements, Velocities;



    # -----------------------------------------------------------------------------------
    # 13 + 1/2. Save the final displacemt, velocity

    velo_name = "velocity_s%f_%02d" % (s, myid)
    pos_name = "position_s%f_%02d" % (s, myid)
    ...
    if (offline):
        ...
        parallel_mesh.Print(mesh_name, 8)
        parallel_mesh.SwapNodes(nodes, owns_nodes)
